...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%开始识别%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fo = fopen('./recognitionResult/f10d1_90/ZN/recognizeResult_1factor1f.txt','wt');
fprintf(fo,'%s\t%s\n','特征参数','识别率');

for thisFeature = 1:featureKind
thisPath = strcat('./recognitionResult/f10d1_90/ZN/singleFeatureResult_1factor',num2str(thisFeature),'tf.txt');
singleFo = fopen(thisPath,'wt');
fprintf(singleFo,'%s\t%s\n','识别种类','实际种类');
rightCnt = 0;
...
for i = 1: size(testcaseFiles,2)
	...
	f2 = importdata([dataPath '\newall_' num2str(firstCouple.couple{2}) 'ZN' '.txt']);
		...
		%line = thisFeature+(firstCouple.location(thisFeature)-1)*featureKind;
		line = thisFeature+featureStart-1+(firstCouple.location(thisFeature)-1)*totalFeatureKind;
		tdata = ft(line,:);
		data1 = f1(line,:);
		data2 = f2(line,:);
		...
		distance1 = (abs(aver1-tdata))/varia1;
		distance2 = (abs(aver2-tdata))/varia2;
	...
	if(minUSD==str2num(trueKind))
		fprintf(singleFo,'%3.0f\t\t%s\n',minUSD,trueKind);
		rightCnt = rightCnt+1;
	else	
		fprintf(singleFo,'%3.0f\t\t%s\t%s\n',minUSD,trueKind,fileName);
	end
	
end

fprintf(singleFo,'\n','');
fprintf(singleFo,'%s\t%3.0f\n','正确识别数：',rightCnt);
fprintf(singleFo,'%s\t%3.0f\n','总数为：',size(testcaseFiles,2));
accuracyRate = rightCnt/size(testcaseFiles,2);
disp('准确率为：');
disp(accuracyRate);
fprintf(singleFo,'%s\t','准确率为：');
fprintf(singleFo,'%3.6f\n',accuracyRate);
fclose(singleFo);

fprintf(fo,'%s\t',titleArray{thisFeature});
fprintf(fo,'%3.6f\n',accuracyRate);
end
fclose(fo);
fclose('all');
