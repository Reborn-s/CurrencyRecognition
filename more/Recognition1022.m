...
dataPath = '..\newall\f10d1\ZY';
testcasePath = '../matlabTestcase/f10d1/ZY/';	%待修改
paramPath = './recognitionResult/f10d1/ZY/minLocation_f10d1_1factor.txt';
...
fo = fopen('./recognitionResult/f10d1/ZY/recognizeResult_1factorEhenced.txt','wt');
fprintf(fo,'%s\t%s\n','识别种类','实际种类');
rightCnt = 0;
...
for i = 1: size(testcaseFiles,2)
	...
	for f = 1: featureKind
		...
		if(f==2||f==7)
			distance1 = distance1+1.5*(abs(aver1-tdata))/varia1;
			distance2 = distance2+1.5*(abs(aver2-tdata))/varia2;
		else
