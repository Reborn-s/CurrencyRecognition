...
dollarArray = {'5FY';'10FY';'20FY';'50FY';'100FY'};
dataPath = '..\newall\f10d5\FY\';
outputPath = './recognitionResult/f10d5/FY/'
% dataPath = '..\newall\f5d1\grayEqualization\FY\';
% outputPath = './recognitionResult/f5d1_ori/grayEqualization/FY/'
coupleCnt = 1;
interval = 100;
%factor = 1;
factor = 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%开始处理%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fo = fopen([outputPath '每对美金特征值相差最大位置f10d5_1factor.txt'],'wt');
fo1 = fopen([outputPath 'minLocation_f10d5_1factor.txt'],'wt');
fprintf(fo,'%s\t%s\t%s\t%s\t%s\n','美金1','美金2','特征参数','最小位子','相交面积');

for i = 1: USDKind-1
	fi  = importdata([dataPath 'newall_' dollarArray{i} '.txt']);
	for j = i+1: USDKind
		disp(dollarArray{i});
		disp(dollarArray{j});
		...
		fj = importdata([dataPath 'newall_' dollarArray{j} '.txt']);
		
		line = 1;
		feature = 1;
		location = 1;
		while(line<=size(fi,1)&&line<=size(fj,1))
			if(line~=1&&mod(line,featureKind)==1)
				location = location+1
				feature = 1;
			end
